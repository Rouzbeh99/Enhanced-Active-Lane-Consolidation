...
private:
    ...
    Value *initialStepVector;
    Value *StepVectorUpdateValue;
    Value *LoopStepVector;
...
private:
    Value *
    formPredicateVector(IRBuilder<> &IRB, BasicBlock *decisionBlock,
                        BasicBlock *vectorizingBlock, BasicBlock *targetedBlock,
                        std::map<const Value *, const Value *> **outputVMap);

private:
    void vectorizeTargetedBlockInstructions(
            BasicBlock *vectorizingBlock, BasicBlock *targetedBlock,
            Value *predicates,
            std::map<const Value *, const Value *> *headerInstructionsMap);

private:
    void fillVectorizingBlock(BasicBlock *vectorizingBlock, BasicBlock *preVec,
                              BasicBlock *exitBlock);

private:
    Value *createVectorOfConstants(Value *value, IRBuilder<> &IRB,
                                   std::string name);

private:
    std::map<const Value *, const Value *> *
    vectorizeInstructions_nonePredicated(std::vector<Instruction *> *instructions,
                                         BasicBlock *block,
                                         ValueToValueMapTy &inputMap);

private:
    void vectorizeInstructions_Predicated(
