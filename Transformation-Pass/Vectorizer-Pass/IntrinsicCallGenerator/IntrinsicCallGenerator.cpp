...
Value *IntrinsicCallGenerator::createAllTruePredicates(IRBuilder<> &IRB) {
    ...
    auto *constantInt = llvm::ConstantInt::get(IRB.getInt32Ty(), VF, /*Scalable*/ true);
...
Value *IntrinsicCallGenerator::createStepVector64Intrinsic(IRBuilder<> &IRB) {
    return IRB.CreateStepVector(VectorType::get(IRB.getInt64Ty(), VF, /*Scalable*/ true));
}

Value *IntrinsicCallGenerator::createStepVector32Intrinsic(IRBuilder<> &IRB) {
    return IRB.CreateStepVector(VectorType::get(IRB.getInt32Ty(), VF, /*Scalable*/ true));
}
