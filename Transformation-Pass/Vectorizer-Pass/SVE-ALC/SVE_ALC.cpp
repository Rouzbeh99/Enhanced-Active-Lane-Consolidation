...
void SVE_ALC::doTransformation() {
    ...
    std::vector<Value *> *uniformBlockOutputs = fillUniformBlock(uniformBlock, joinBlock, findTargetedBlock(), header,
                                                                 ...
                                                                 (*headerOutputs)[0]);
    fillLinearizedBlock(linearizedBlock, newLatch, findTargetedBlock(), inductionVar, (*headerOutputs)[0]);
    std::vector<Value *> *joinBlockOutputs = fillJoinBlock(joinBlock, newLatch, uniformBlock, laneGatherBlock,
                                                           headerOutputs, uniformBlockOutputs);
    fillNewLatchBlock(newLatch, alcHeader, middleBlock, preheaderForRemainingBlock, joinBlock, linearizedBlock,
                      headerOutputs, joinBlockOutputs, preALCBlockValues->back());
...
// TODO: problem in algorithm, in the case where the total number of active lanes is not enough to fill a vector, the result vector is not correct
    ...
    SVE_ALC::fillPreALCBlock(BasicBlock *preALCBlock, BasicBlock *preheader, BasicBlock *alcHeader) {
        ...
        // create step vector
        Value *initialUniformVec = nullptr;
        Value *stepVal = nullptr;

        allTrue = intrinsicCallGenerator->createAllTruePredicates(builder);

        if (tripCount->getType() == Type::getInt64Ty(preheader->getContext())) {
            initialUniformVec = intrinsicCallGenerator->createStepVector64Intrinsic(builder, "uniform.vector");
        } else {
            initialUniformVec = intrinsicCallGenerator->createStepVector32Intrinsic(builder, "uniform.vector");
        }
        stepVal = vectorLength;
    ...
    SVE_ALC::fillALCHeaderBlock(BasicBlock *alcHeader, BasicBlock *laneGatherBlock, BasicBlock *linearized,
        ...
        index->addIncoming(contZero, preALC);
        ...
        Value *indexVector = intrinsicCallGenerator->createIndexInstruction(builder, index, indexUpdateValue);
    ...
    SVE_ALC::fillLaneGatherBlock(BasicBlock *laneGather, BasicBlock *alcApplied, BasicBlock *joinBlock, Value *z0,
        ...
        Value *condition = builder.CreateICmpUGE(bothActives, vectorLength);
    ...
    void SVE_ALC::fillNewLatchBlock(BasicBlock *newLatch, BasicBlock *alcHeader, BasicBlock *middleBlock,
                                    BasicBlock *preHeaderForRemaining, BasicBlock *joinBlock,
        ...
        Value *condition = builder.CreateICmpUGT(updatedIndex, totalVecIterations);
    ...
    SVE_ALC::fillJoinBlock(BasicBlock *joinBlock, BasicBlock *newLatch, BasicBlock *uniformBlock,
        ...
        Value *&oldIndex = (*alcHeaderOutputs)[0];
        Value *&oldUniformVec = (*alcHeaderOutputs)[1];
        Value *&oldPredicates = (*alcHeaderOutputs)[2];
        Value *&oldActiveLanesCount = (*alcHeaderOutputs)[5];
        ...
        PHINode *indexPhi = builder.CreatePHI(oldIndex->getType(), 2);
        indexPhi->addIncoming(oldIndex, laneGather);
        ...
        PHINode *activeLanesCountPhi = builder.CreatePHI(oldActiveLanesCount->getType(), 2);
        activeLanesCountPhi->addIncoming(oldActiveLanesCount, laneGather);
